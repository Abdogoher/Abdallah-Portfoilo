  ...
  return (
    ...
    >
      <motion.div
        ...
        className="flex-1 order-2 sm:order-1 text-center md:text-left rtl:text-right"
      >
        <h1
          id="hero-heading"
          className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 bg-linear-to-r from-blue-900 via-slate-700 to-slate-600 dark:from-blue-400 dark:via-slate-400 dark:to-slate-300 bg-clip-text text-transparent leading-tight pb-2"
        >
          {t("name")}
        </h1>
        ...
        {/* buttons  */}
        <div className="flex flex-wrap gap-4 justify-center md:justify-start rtl:md:justify-start">
          <Link
            href={`/${locale}/projects`}
            className="px-6 py-3 bg-linear-to-r from-blue-900 to-slate-700 text-white rounded-lg hover:from-blue-950 hover:to-slate-800 shadow-lg hover:shadow-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:ring-offset-2 font-medium"
          >
            {t("viewProjects")}
          </Link>
          <a
      ...
      >
        <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96">
          <div
            className="absolute inset-0 bg-linear-to-br from-blue-800 via-slate-600 to-slate-700 rounded-full blur-2xl opacity-30 animate-pulse"
            aria-hidden="true"
          ></div>
          <Image
            alt={`${t("profileImageAlt")} || ${t("name")}`}
            fill
            priority
            className="rounded-full object-cover shadow-2xl relative z-10 border-4 border-white dark:border-slate-800"
            src="/Abdallah_avatar.jpeg"
